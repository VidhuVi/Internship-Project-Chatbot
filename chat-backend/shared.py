...
from starlette.concurrency import run_in_threadpool # NEW: Import for async operations
...
async def extract_text_from_docx(file_stream: io.BytesIO) -> list[str]:
    ...
    full_text_content = []
    try:
        document = await run_in_threadpool(Document, file_stream) # Wrap Document init
        for paragraph in document.paragraphs:
            # Text extraction is generally not blocking, but for consistency/safety
            # if there were complex properties being accessed.
